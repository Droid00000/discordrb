...
module Discordrb
  ...
  class Gateway
    ...
    # Member count before a guild is considered to be large.
    LARGE_THRESHOLD = 250
    ...
    def connected?
      @websocket&.connected? || !@session.nil? || false
    end
    ...
    # @return [true, false, nil] Whether or not the event was successfully sent.
    def request_channel_info(guild:, fields:)
      options = {
        guild_id: guild.resolve_id,
    ...
    # @return [true, false, nil] Whether or not the event was successfully sent.
    def request_soundboard_sounds(guilds:)
      options = { guild_ids: [*guilds].map(&:resolve_id) }
    ...
    def send_command(opcode, inner_payload)
      @websocket&.send(JSON.dump({ op: opcode, d: inner_payload }))
    end
    ...
    def notify_message(message)
      ...
      when OPCODES[:heartbeat]
        send_command(OPCODES[:heartbeat], @session&.sequence)
    ...
    def initialize_heartbeats(data)
      ...
      @heartbeat_manager = Thread.new do
        ...
        # The first heartbeart is special. We have to wait a
        # jitter value, before sending a beat. The Discord devs require
        # this in order to prevent too many clients from reconnecting their
        # sessions at the same time and creating a "thundering herd" effect.
