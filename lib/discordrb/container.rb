...
module Discordrb
  ...
  module EventContainer
    ...
    # This **event** is raised whenever an effect such is played in a voice channel the bot is connected to.
    # @param attributes [Hash] The event's attributes.
    # @option attributes [String, Integer, Server] :server A server to match against.
    # @option attributes [String, Integer, Channel] :channel A channel to match against.
    # @option attributes [Integer, Symbol] :animation_type An animation type to match against.
    # @option attributes [Integer, String, Member] :member A member to match against (you can also pass `:user`).
