...
module Discordrb::Events
  ...
  class AutoModRuleEvent < Event
    ...
    def initialize(data, bot)
      ...
      @automod_rule = @guild.automod_rule(data[:id].to_i)
    end
  end
  ...
  class AutoModRuleDeleteEvent < AutoModRuleEvent
    ...
    def initialize(data, bot)
      @bot = bot
      @guild = bot.guild(data[:guild_id].to_i)
      @automod_rule = Discordrb::AutoModRule.new(data, @guild, @bot)
    end
  end
  ...
  class AutoModRuleExecutionEvent < Event
    ...
    # @return [Integer] the ID of the guild where the offending content was made.
    ...
    # Get the guild where the offending content was made.
    # @return [Guild] The guild containing the offending content.
    def guild
    ...
    # @return [AutoModRule] The auto moderation rule associated with the event.
    def automod_rule
      @automod_rule ||= guild.automod_rule(@automod_rule_id)
    end
