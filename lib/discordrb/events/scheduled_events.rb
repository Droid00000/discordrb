...
module Discordrb::Events
  ...
  class ScheduledEventEvent < Event
    ...
    def initialize(data, bot)
      ...
      @scheduled_event = @guild&.scheduled_event(data[:id].to_i)
    end
  end
  ...
  class ScheduledEventDeleteEvent < ScheduledEventEvent
    ...
    def initialize(data, bot)
      @bot = bot
      @guild = bot.guild(data[:guild_id].to_i)
      @scheduled_event = Discordrb::ScheduledEvent.new(data, @guild, @bot)
    end
  end
  ...
  class ScheduledEventUserEvent < Event
    ...
    # Get the guild the scheduled event in question is from.
    # @return [Guild] the guild the scheduled event is associated with.
    def guild
      @bot.guild(@guild_id)
    end
    ...
    # @return [Member, User] the guild member that was added to or removed from the
