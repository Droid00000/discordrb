...
module Discordrb
  ...
  class Bot
    ...
    #     end
    #   end
    def register_application_command(name, description, server_id: nil, default_permission: nil, type: :chat_input, default_member_permissions: nil, contexts: nil, name_localizations: nil, description_localizations: nil, integration_types: nil)
      ...
      contexts&.map! { |context| Interaction::CONTEXTS[context] || context }
      integration_types&.map! { |integration| Interaction::INTEGRATION_TYPES[integration] || integration }

      resp = if server_id
               API::Application.create_guild_command(@token, profile.id, server_id, name, description, builder.to_a, default_permission, type, default_member_permissions&.to_s, contexts, name_localizations, description_localizations)
             else
               API::Application.create_global_command(@token, profile.id, name, description, builder.to_a, default_permission, type, default_member_permissions&.to_s, contexts, name_localizations, description_localizations, integration_types)
             end
    ...
    # @yieldparam [OptionBuilder]
    # @yieldparam [PermissionBuilder]
    def edit_application_command(command_id, server_id: nil, name: nil, description: nil, default_permission: nil, type: :chat_input, default_member_permissions: nil, contexts: nil, name_localizations: nil, description_localizations: nil, integration_types: nil)
      type = ApplicationCommand::TYPES[type] || type
      ...
      resp = if server_id
               API::Application.edit_guild_command(@token, profile.id, server_id, command_id, name, description, builder.to_a, default_permission, type, default_member_permissions&.to_s, contexts, name_localizations, description_localizations)
             else
               API::Application.edit_global_command(@token, profile.id, command_id, name, description, builder.to_a, default_permission, type, default_member_permissions&.to_s, contexts, name_localizations, description_localizations, integration_types)
             end
      cmd = ApplicationCommand.new(JSON.parse(resp), self, server_id)
    ...
    # @param name [String] The name of the application emoji to create.
    # @param image [String, #read] Base64 string with the image data, or a file-like object that responds to #read.
    # @return [Emoji] The application emoji that has been created.
    ...
    def handle_dispatch(type, data)
      ...
      when :GUILD_MEMBERS_CHUNK
        ...
        server = server(id)
        server.process_chunk(data['members'], data['chunk_index'], data['chunk_count'], data['not_found'])
      when :INVITE_CREATE
