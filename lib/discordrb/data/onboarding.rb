...
module Discordrb
  ...
  class Onboarding
    ...
    # @return [Guild] the guild the onboarding flow is from.
    attr_reader :guild
    ...
    # @return [String] a string that can be used to mention the onboarding tab.
    def mention
      '<id:customize>'
    end
    ...
    # @param in_onboarding [true, false] Whether or not the prompt is present in the onboarding flow. If set
    #   to `false`, the prompt will only appear in the "Channels & Roles" tab.
    # @param options [Array<#to_h>, nil] The selectable answers to the prompt. There's also a yielded builder.
    # @param reason [String, nil] The reason to show in the guild's audit log for creating the onboarding prompt.
    # @yieldparam builder [OptionBuilder] An optional option builder. Overrides the `options:` argument if passed.
    # @return [nil]
    def create_prompt(
      title:, required: false, type: :grid, single_select: true,
      in_onboarding: true, options: nil, reason: nil
    )
      yield((builder = OptionBuilder.new(@guild))) if block_given?
    ...
    # Modify the properties of the onboarding process.
    ...
    # @param reason [String, nil] The reason to show in the guild's audit log for modifying the onboarding.
    # @return [nil]
    def modify(
      mode: :undef, prompts: :undef, enabled: :undef, default_channels: :undef,
      reason: nil
    )
      data = {
      ...
      update_data(@bot.http.modify_guild_onboarding(@guild.id, **data, reason: reason))
      nil
    end

    # @!method default_mode?
    #   @return [true, false] whether onboarding will only count default channels toward constraints.
    # @!method advanced_mode?
    #   @return [true, false] whether onboarding will count default channels **and** questions toward constraints.
    ...
    # @!visibility private
    def inspect
      "<Onboarding guild_id=#{@guild.id} mode=#{@mode} enabled=#{@enabled || 'false'}>"
    end
    ...
    class Prompt
      ...
      # @return [Guild] The guild that the onboarding prompt is from.
      def guild
        @onboarding.guild
      end
      ...
      # @param reason [String, nil] The reason to show in the guild's audit log for modifying the prompt.
      # @return [nil]
      def modify(
      ...
      # @param roles [Array<Integer, Role, String>, nil] The roles of the option.
      # @param channels [Array<Integer, Channel, String>, nil] The channels of the option.
      # @param reason [String, nil] The reason to show in the audit log for creating the option.
      # @return [nil]
      def create_option(
        title:, description: nil, emoji: nil, roles: nil,
        channels: nil, reason: nil
      )
        data = {
          title: title,
          ...
          **(emoji ? Option.emoji_hash(emoji, guild) : {})
        }
      ...
      # @param reason [String, nil] The reason to show in the guild's audit log for deleting the prompt.
      # @return [nil]
      def delete(reason: nil)
        prompts = @onboarding.prompts.dup

        @onboarding.modify(prompts: prompts.tap { |list| list.delete(@id) }, reason: reason)
      end

      # @!method grid?
      #   @return [true, false] whether or not the prompt is rendered as a clickable grip of answers.
      # @!method dropdown?
      ...
      # @!visibility private
      def inspect
        "<Prompt id=#{@id} type=#{@type} title=\"#{@title}\">"
      end
    ...
    class Option
      ...
      def emoji
        @emoji&.id ? @prompt.guild.emoji(@emoji.id) : @emoji
      end
      ...
      def roles
        @role_ids.filter_map { |role_id| @prompt.guild.role(role_id) }
      end
      ...
      def modify(
        ...
        data = {
          ...
          channel_ids: channels == :undef ? @channel_ids : [*channels].map(&:resolve_id),
          role_ids: roles == :undef ? @role_ids : [*roles].map(&:resolve_id),
          **(emoji == :undef ? Option.emoji_hash(@emoji) : Option.emoji_hash(emoji, @prompt.guild))
        }
      ...
      # @param reason [String, nil] The reason to show in the guild's audit log for deleting the option.
      # @return [nil]
      def delete(reason: nil)
      ...
      # @!visibility private
      def self.emoji_hash(emoji, guild = nil)
        if emoji.nil?
          {
            emoji_id: nil,
            emoji_name: nil,
        ...
        elsif emoji.is_a?(Emoji)
          {
            emoji_id: emoji.id,
            emoji_name: emoji.name,
            emoji_animated: emoji.animated?
          }
        elsif emoji.to_i.zero?
          {
            emoji_id: nil,
            emoji_name: emoji,
            emoji_animated: false
          }
        elsif emoji.to_i.positive?
          resolved = guild.emoji(emoji.to_i)

          {
            emoji_id: resolved.id,
            emoji_name: resolved.name,
            emoji_animated: resolved.animated?
          }
        end
      end
    end
    ...
    class OptionBuilder
      ...
      # @param emoji [Emoji, Integer, String, nil] The emoji object, string for a unicode emoji, or nil for no emoji.
      # @return [void]
