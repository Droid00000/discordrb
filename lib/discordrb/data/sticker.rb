# frozen_string_literal: true

module Discordrb
  # A sticker that can be sent in a message.
  class Sticker
    include Snowflake

    # Map of format types.
    FORMATS = {
      png: 1,
      apng: 2,
      lottie: 3,
      gif: 4
    }.freeze

    # @return [String] the 2-30 character name of the sticker.
    attr_reader :name

    # @return [String] the 1-200 character tags of the sticker.
    attr_reader :tags

    # @return [true, false] whether or not the sticker can be used.
    attr_reader :available
    alias available? available

    # @return [Integer, nil] the ID of the guild the sticker is from.
    attr_reader :guild_id

    # @return [Integer] The sort order of a standard sticker within its pack.
    attr_reader :sort_order

    # @return [String, nil] the 1-100 character description of the sticker, or `nil`.
    attr_reader :description

    # @return [Integer] the type of the sticker's file, e.g. `png`, `apng`, `gif` etc.
    attr_reader :format_type

    # @!visibility private
    def initialize(data, guild, bot)
      @bot = bot
      @guild = guild
      @id = data[:id].to_i
      @type = data[:type]
      @pack_id = data[:pack_id]&.to_i
      @format_type = data[:format_type]
      @sort_order = data[:sort_value] || 0
      @guild_id = @guild&.id || data[:guild_id]&.to_i
      update_data(data)
    end

    # Whether the sticker is an official sticker in a pack.
    # @return [true, false] Whether the sticker is an official sticker in a pack.
    def official?
      @type == 1
    end

    # Whether the sticker is a custom sticker uploaded to a guild.
    # @return [true, false] Whether the sticker is a custom sticker that was uploaded to a guild.
    def custom?
      @type == 2
    end

    alias_method :guild?, :custom?
    alias_method :default?, :official?

    # Modify the properties of the sticker.
    # @param name [String] The new 2-30 character name of the sticker.
    # @param tags [String, Array<String>] The new tags of the sticker, max 200 characters.
    # @param description [String, nil] The new 2-100 character description of the sticker.
    # @param reason [String, nil] The reason to show in the audit log for modifying the sticker.
    # @return [nil]
    def modify(name: :undef, description: :undef, tags: :undef, reason: nil)
      raise Discordrb::Errors::NoPermission, 'Cannot update a default sticker' if default?

      data = {
        name: name,
        description: description,
        tags: tags.is_a?(Array) ? tags.join(', ') : tags
      }

      update_data(@bot.http.modify_guild_sticker(@guild_id, @id, **data, reason: reason))
      nil
    end

    # Get the sticker pack the sticker is associated with.
    # @return [Pack, nil] The pack this sticker is from, or `nil` if the sticker doesn't have an associated pack.
    def pack
      @bot.sticker_pack(@pack_id) if @pack_id
    end

    # Get the guild the sticker is associated with.
    # @return [Guild, nil] The guild the sticker is associated with, or `nil` if the guild does not exist.
    def guild
      (@guild ||= @bot.guild(@guild_id)) if @guild_id
    end

    # Get the user who uploaded the sticker.
    # @return [User, nil] The user who uploaded the sticker, or `nil` if the creator could not be resolved.
    def creator
      return @creator if @creator || official?

      return unless guild&.bot&.can_manage_expressions? || guild&.bot&.can_create_expressions?

      update_data(@bot.http.get_guild_sticker(@guild_id, @id))
      @creator
    rescue Discordrb::Errors::NoPermission, Discordrb::Errors::NotFound
      nil
    end

    # Delete the sticker.
    # @param reason [String, nil] The reason to show in the audit log for deleting the sticker.
    # @return [nil]
    def delete(reason: nil)
      raise Discordrb::Errors::NoPermission, 'Cannot delete a default sticker' if default?

      @bot.http.delete_guild_sticker(@guild_id, @id, reason: reason)
      @guild&.delete_sticker(@id)
      nil
    end

    # Get the extension that can be used to access the sticker's URL.
    # @return [String] The file extension of the sticker. Will be one of `png`, `json`, or `gif`.
    def extension
      if png? || apng?
        'png'
      elsif lottie?
        'json'
      elsif gif?
        'gif'
      end
    end

    # Utility method to get a sticker's CDN URL.
    # @param query [true, false] Whether or not to append
    #   the size query parameter for `gif` stickers.
    # @return [String] The CDN URL to the sticker's file.
    def url(query: true)
      if gif?
        Assets[:gif_sticker, @id, extension, size: query ? 4096 : nil]
      else
        Assets[:sticker, @id, extension]
      end
    end

    # @!method png?
    #   @return [true, false] whether or not the sticker is a PNG file.
    # @!method apng?
    #   @return [true, false] whether or not the sticker is an animated APNG file.
    # @!method lottie?
    #   @return [true, false] whether or not the sticker is a lottie JSON file.
    # @!method gif?
    #   @return [true, false] whether or not the sticker is an animated GIF file.
    FORMATS.each do |name, value|
      define_method("#{name}?") do
        @format_type == value
      end
    end

    # @!visibility private
    def inspect
      "<Sticker id=#{@id} name=\"#{@name}\" format_type=#{@format_type}>"
    end

    # @!visibility private
    def update_data(new_data)
      @name = new_data[:name]
      @tags = new_data[:tags]
      @available = custom? ? new_data[:available] : true
      @creator = @bot.ensure_user(new_data[:user]) if new_data[:user]
      @description = new_data[:description] == '' ? nil : new_data[:description]
    end

    # A pack of official stickers that everyone can use.
    class Pack
      include Snowflake

      # @return [String] the name of the sticker pack.
      attr_reader :name

      # @return [Integer] the ID of the sticker pack's SKU.
      attr_reader :sku_id

      # @return [String, nil] the banner hash of the sticker pack.
      attr_reader :banner

      # @return [Array<Sticker>] the stickers in the sticker pack.
      attr_reader :stickers

      # @return [String] the description of the sticker pack shown in the store.
      attr_reader :description

      # @return [Sticker, nil] the sticker that's shown on the sticker pack's icon.
      attr_reader :cover_sticker

      # @!visibility private
      def initialize(data, bot)
        @bot = bot
        @id = data[:id].to_i
        @name = data[:name]
        @sku_id = data[:sku_id].to_i
        @banner = data[:banner_asset_id]
        @description = data[:description]
        @stickers = data[:stickers].map { |sticker| @bot.ensure_default_sticker(sticker) }
        @cover_sticker = @stickers.find { |sticker| sticker.id == data[:cover_sticker_id]&.to_i }
      end

      # Utility method to get a sticker pack's banner URL.
      # @param format [String] The URL will default to `webp`. You can otherwise specify one of `webp`, `jpg` or `png` to override this.
      # @param size [Integer, nil] The URL will default to `4096`. You can otherwise specify any number that's a power of two to override this.
      # @return [String, nil] The URL to the sticker pack's banner image, or `nil` if the sticker pack doesn't have an associated banner image.
      def banner_url(format: 'webp', size: 4096)
        Assets[:sticker_pack_banner, @banner, format, size:] if @banner
      end

      # @!visibility private
      def inspect
        "<Pack id=#{@id} name=\"#{@name}\">"
      end
    end

    # The smallest amount of data required to render a sticker.
    class Item
      include Snowflake

      # @return [String] the name of the sticker item.
      attr_reader :name

      # @return [Integer] the format type of the sticker item.
      attr_reader :format_type

      # @!visibility private
      def initialize(data, bot)
        @bot = bot
        @id = data[:id].to_i
        @name = data[:name]
        @format_type = data[:format_type]
      end

      # @!method png?
      #   @return [true, false] whether or not the sticker item is a PNG file.
      # @!method apng?
      #   @return [true, false] whether or not the sticker item is an animated APNG file.
      # @!method lottie?
      #   @return [true, false] whether or not the sticker item is a lottie JSON file.
      # @!method gif?
      #   @return [true, false] whether or not the sticker item is an animated GIF file.
      FORMATS.each do |name, value|
        define_method("#{name}?") do
          @format_type == value
        end
      end

      # Get the extension that can be used to access the sticker's URL.
      # @return [String] The file extension of the sticker. Will be one of `png`, `json`, or `gif`.
      def extension
        if png? || apng?
          'png'
        elsif lottie?
          'json'
        elsif gif?
          'gif'
        end
      end

      # Utility method to get a sticker item's CDN URL.
      # @param query [true, false] Whether or not to append the
      #   size query parameter for `gif` sticker items.
      # @return [String] The CDN URL to the sticker item's file.
      def url(query: true)
        if gif?
          Assets[:gif_sticker, @id, extension, size: query ? 4096 : nil]
        else
          Assets[:sticker, @id, extension]
        end
      end

      # Convert the sticker item into a full sticker object.
      # @return [Sticker, nil] The full sticker object, or `nil` if the sticker was deleted.
      def to_sticker(fetch: true)
        @bot.sticker(@id, fetch: fetch)
      end

      # @!visibility private
      def inspect
        "<Item id=#{@id} name=\"#{@name}\" format_type=#{@format_type}>"
      end
    end
  end
end
