...
module Discordrb
  # The welcome screen for a guild.
  class WelcomeScreen
    # @return [Guild] the guild the welcome screen is for.
    attr_reader :guild
    ...
    def modify(enabled: :undef, channels: :undef, description: :undef, reason: nil)
      data = {
        enabled: enabled,
        description: description,
        welcome_channels: channels == :undef ? channels : channels&.map(&:to_h)
      }

      update_data(@bot.http.modify_guild_welcome_screen(@guild.id, **data, reason:))
      nil
    end
    ...
    # @!visibility private
    def inspect
      "<WelcomeScreen guild_id=#{@guild.id} description=\"#{@description}\">"
    end
    ...
    class WelcomeChannel
      ...
      # @param reason [String, nil] The reason to show in the guild's audit for modifying the channel.
      # @return [nil]
      def modify(description: :undef, emoji: :undef, reason: nil)
        ...
        if emoji == :undef
          channel_data.merge!(Emoji.build_hash(self.emoji))
        end
