...
module Discordrb
  ...
  class Interaction
    ...
    # @param poll [Hash, nil, Discordrb::Poll::Builder] A poll request object to include with this message.
    ...
    def respond(content: nil, tts: nil, embeds: nil, allowed_mentions: nil, flags: 0, ephemeral: nil, wait: false, components: nil, poll: nil)
      ...
      Discordrb::API::Interaction.create_interaction_response(@token, @id, CALLBACK_TYPES[:channel_message], data[:content], tts, data[:embeds],
                                                              data[:allowed_mentions], flags, components.to_a, data[:poll])
    ...
    def update_message(content: nil, tts: nil, embeds: nil, allowed_mentions: nil, flags: 0, ephemeral: nil, wait: false, components: nil, poll: nil)
      ...
      Discordrb::API::Interaction.create_interaction_response(@token, @id, CALLBACK_TYPES[:update_message], data[:content], tts, data[:embeds],
                                                              data[:allowed_mentions], flags, components.to_a, data[:poll])
    ...
    def edit_response(content: nil, embeds: nil, allowed_mentions: nil, components: nil, poll: nil)
      ...
      components ||= view
      data = builder.to_json_hash
      resp = Discordrb::API::Interaction.edit_original_interaction_response(@token, @application_id, data[:content], data[:embeds],
                                                                            data[:allowed_mentions], components.to_a, data[:poll])
    ...
    def send_message(content: nil, embeds: nil, tts: false, allowed_mentions: nil, flags: 0, ephemeral: false, components: nil, poll: nil)
      ...
      resp = Discordrb::API::Webhook.token_execute_webhook(
        @token, @application_id, true, data[:content], nil, nil, tts, nil, data[:embeds], data[:allowed_mentions], flags, components.to_a, data[:poll]
      )
      Interactions::Message.new(JSON.parse(resp), @bot, @interaction)
    end
    ...
    # @param poll [Hash, nil, Discordrb::Poll::Builder] A poll request object to include with this message.
    # @yieldparam builder [Webhooks::Builder] An optional message builder. Arguments passed to the method overwrite builder data.
    def edit_message(message, content: nil, embeds: nil, allowed_mentions: nil, components: nil, poll: nil)
      builder = Discordrb::Webhooks::Builder.new
      view = Discordrb::Webhooks::View.new
      ...
      resp = Discordrb::API::Webhook.token_edit_message(
        @token, @application_id, message.resolve_id, data[:content], data[:embeds], data[:allowed_mentions], components.to_a, data[:poll]
      )
      Interactions::Message.new(JSON.parse(resp), @bot, @interaction)
    end
    ...
    # @param poll [Hash, Discordrb::Poll::Builder, nil]
