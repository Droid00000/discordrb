# frozen_string_literal: true

module Discordrb
  # An auto moderation rule on a guild.
  class AutoModRule
    include Snowflake

    # Mapping of event types.
    EVENT_TYPES = {
      message_send: 1,
      member_update: 2
    }.freeze

    # @return [String] the name of the auto moderation rule.
    attr_reader :name

    # @return [Guild] the guild of the auto moderation rule.
    attr_reader :guild

    # @return [Trigger] the trigger of the auto moderation rule.
    attr_reader :trigger

    # @return [Array<Action>] the actions of the auto moderation rule.
    attr_reader :actions

    # @return [true, false] whether or not the auto moderation rule is enabled.
    attr_reader :enabled
    alias enabled? enabled

    # @return [Integer] the type of event that can trigger the auto moderation rule.
    # @see EVENT_TYPES
    attr_reader :event_type

    # @!visibility private
    def initialize(data, guild, bot)
      @bot = bot
      @guild = guild
      @id = data[:id].to_i
      @creator_id = data[:creator_id].to_i
      update_data(data)
    end

    # Get the user who was responsible for creating the auto moderation rule.
    # @return [User] The user who was responsible for creating the auto moderation rule.
    def creator
      @bot.user(@creator_id)
    end

    # Get the roles that will be ignored by the auto moderation rule.
    # @return [Array<Role>] The roles that should be ignored by the auto moderation rule.
    def exempt_roles
      @exempt_roles.filter_map { |id| @guild.role(id) }
    end

    # Get the channels that will be ignored by the auto moderation rule.
    # @return [Array<Channel>] The channels that should be ignored by the auto moderation rule.
    def exempt_channels
      @exempt_channels.filter_map { |id| @bot.channel(id) }
    end

    # Check if the auto moderation rule can be triggered via a message event.
    # @return [true, false] Whether or not the auto moderation rule will trigger when a message is sent or edited.
    def message_event?
      @event_type == EVENT_TYPES[:message_send]
    end

    # Check if the auto moderation rule can be triggered via a member event.
    # @return [true, false] Whether or not the auto moderation rule will trigger when a member edits their profile.
    def member_event?
      @event_type == EVENT_TYPES[:member_update]
    end

    # Modify the properties of the auto moderation rule.
    # @param name [String] The new 1-100 character name of the auto moderation rule.
    # @param event_type [Symbol, Integer] The new event type of the auto moderation rule; see {EVENT_TYPES}.
    # @param enabled [true, false] Whether or not the auto moderation rule should be enabled.
    # @param actions [Array<#to_h, Action>] The new actions to set for the auto moderation rule.
    # @param exempt_roles [Array<Integer, String, Role>] The new roles that should be ignored by the auto moderation rule.
    # @param exempt_channels [Array<Integer, String, Channel>] The new channels that should be ignored by the auto moderation rule.
    # @param mention_limit [Integer] The new max amount of role and user mentions allowed per message; max 50.
    # @param keyword_filter [Array<String>, nil] The new substrings which will be searched for in content; max 1000.
    # @param regex_patterns [Array<String>, nil] The new regular expression patterns (rust flavoured) to match against in content; max 10.
    # @param exempt_keywords [Array<String>, nil] The new substrings which should not trigger the auto moderation rule; max 1000.
    # @param keyword_presets [Array<Integer, Symbol>, nil] The new pre-defined set of keywords to match against in content; see {Trigger::PRESET_TYPES PRESET_TYPES}.
    # @param mention_raid_protection [true, false, nil] Whether or not to automatically detect when a mention raid is occuring.
    # @param reason [String, nil] The reason to show in the guild's audit log for updating the auto moderation rule.
    # @yieldparam builder [Action::Builder] An optional builder for auto moderation actions. Overrides the `actions:` argument if passed.
    # @return [nil]
    def modify(
      name: :undef, event_type: :undef, enabled: :undef, actions: :undef, exempt_roles: :undef,
      exempt_channels: :undef, mention_limit: :undef, keyword_filter: :undef, regex_patterns: :undef,
      exempt_keywords: :undef, keyword_presets: :undef, mention_raid_protection: :undef, reason: nil
    )
      data = {
        name: name,
        enabled: enabled,
        event_type: event_type == :undef ? event_type : EVENT_TYPES[event_type] || event_type,
        actions: actions == :undef ? actions : actions.to_a.map(&:to_h),
        exempt_roles: exempt_roles == :undef ? exempt_roles : exempt_roles.map(&:resolve_id),
        exempt_channels: exempt_channels == :undef ? exempt_channels : exempt_channels.map(&:resolve_id)
      }

      partial_params = mention_raid_protection != :undef || exempt_keywords != :undef

      if partial_params || regex_patterns != :undef || keyword_filter != :undef || mention_limit != :undef || keyword_presets != :undef
        data[:trigger_metadata] = {
          mention_total_limit: mention_limit == :undef ? @trigger.mention_limit : mention_limit,
          keyword_filter: keyword_filter == :undef ? @trigger.keyword_filter : (keyword_filter || []),
          regex_patterns: regex_patterns == :undef ? @trigger.regex_patterns : (regex_patterns || []),
          allow_list: exempt_keywords == :undef ? @trigger.exempt_keywords : (exempt_keywords || []),
          presets: (keyword_presets == :undef ? @trigger.keyword_presets : (keyword_presets || []))&.map { |id| Trigger::PRESET_TYPES[id] || id },
          mention_raid_protection_enabled: mention_raid_protection == :undef ? @trigger.mention_raid_protection? : (mention_raid_protection || false)
        }
      end

      if block_given?
        yield((builder = Actions::Builder.new))
        data[:actions] = builder.to_a
      end

      update_data(@bot.http.modify_auto_moderation_rule(@guild.id, @id, **data, reason: reason))
      nil
    end

    # Delete the auto moderation rule. Use this with caution, as it cannot be undone.
    # @param reason [String, nil] The reason to show in the guild's audit log for deleting the auto moderation rule.
    # @return [nil]
    def delete(reason: nil)
      @bot.http.delete_auto_moderation_rule(@guild.id, @id, reason: reason)
      @guild.delete_automod_rule(@id)
      nil
    end

    # Check if a specific role, channel, or member is exempt from the auto moderation rule.
    # @param other [Role, Channel, Member] The role, channel, or member that should be checked.
    # @return [true, false] Whether or not the role, channel, or member is exempt from the auto moderation rule.
    def exempt?(other)
      case other
      when Discordrb::Role
        @exempt_roles.include?(other.id)
      when Discordrb::Channel
        @exempt_channels.include?(other.id)
      when Discordrb::Member
        other.bot_account? || other.can_manage_guild? || @exempt_roles.any? { |role_id| other.role?(role_id) }
      else
        raise ArgumentError, "Unsupported type: '#{other.class}' (expected one of 'Member', 'Role', 'Channel')"
      end
    end

    # @!visibility private
    def inspect
      "<AutoModRule id=#{@id} guild_id=#{@guild_id} enabled=#{@enabled}>"
    end

    # @!visibility private
    def update_data(new_data)
      @name = new_data[:name]
      @enabled = new_data[:enabled]
      @event_type = new_data[:event_type]
      @exempt_roles = new_data[:exempt_roles]&.map(&:to_i) || []
      @exempt_channels = new_data[:exempt_channels]&.map(&:to_i) || []
      @actions = new_data[:actions]&.map { |action| Action.new(action, @bot) } || []
      @trigger = Trigger.new(new_data[:trigger_metadata], new_data[:trigger_type], @bot)
    end

    # How an auto moderation rule can be triggered.
    class Trigger
      # Mapping of trigger types.
      TYPES = {
        keyword: 1,
        harmful_link: 2,
        spam: 3,
        keyword_preset: 4,
        mention_spam: 5,
        member_profile: 6
      }.freeze

      # Mapping of preset types.
      PRESET_TYPES = {
        profanity: 1,
        sexual_content: 2,
        slurs: 3
      }.freeze

      # @return [Integer] the type of the trigger.
      attr_reader :type

      # @return [Integer] the max number of unique mentions allowed per message.
      attr_reader :mention_limit

      # @return [Array<String>] the substrings which will be scanned for in content.
      attr_reader :keyword_filter

      # @return [Array<String>] the regular expression patterns to match against in content.
      attr_reader :regex_patterns

      # @return [Array<String>] the substrings which should not trigger the auto moderation rule.
      attr_reader :exempt_keywords

      # @return [Array<Symbol>] the internal pre-defined set of keyword to match against in content.
      attr_reader :keyword_presets

      # @return [true, false] whether or not to automatically detect when a mention raid is occuring.
      attr_reader :mention_raid_protection
      alias mention_raid_protection? mention_raid_protection

      # @!visibility private
      def initialize(data, type, bot)
        @bot = bot
        @type = type
        @mention_limit = data[:mention_total_limit] || 0
        @keyword_filter = data[:keyword_filter] || []
        @regex_patterns = data[:regex_patterns] || []
        @exempt_keywords = data[:allow_list] || []
        @mention_raid_protection = data[:mention_raid_protection_enabled] || false
        @keyword_presets = data[:presets]&.filter_map { |item| PRESET_TYPES.key(item) } || []
      end

      # @!method keyword?
      #   @return [true, false] whether or not the auto moderation rule can be triggered due to a user defined keyword.
      # @!method harmful_link?
      #   @return [true, false] whether or not the auto moderation rule can be triggered when a harmful link is posted.
      # @!method spam?
      #   @return [true, false] whether or not the auto moderation rule can be triggered due to spam content.
      # @!method keyword_preset?
      #   @return [true, false] whether or not the auto moderation rule can be triggered due to a pre-defined keyword preset.
      # @!method mention_spam?
      #   @return [true, false] whether or not the auto moderation rule can be triggered due to mention spam.
      # @!method member_profile?
      #   @return [true, false] whether or not the auto moderation rule can be triggered due to content in a member's profile.
      TYPES.each do |name, value|
        define_method("#{name}?") { @type == value }
      end

      # Check if two auto moderation trigger objects are equivalent.
      # @param other [Object] The object to compare against for equality.
      # @return [true, false] Whether or not the two objects are equivalent.
      def ==(other)
        return false unless other.is_a?(Trigger)

        @type == other.type && @regex_patterns == other.regex_patterns && @mention_limit == other.mention_limit &&
          @mention_raid_protection == other.mention_raid_protection? && @keyword_filter == other.keyword_filter &&
          @keyword_presets == other.keyword_presets && @exempt_keywords == other.exempt_keywords
      end

      alias_method :eql?, :==

      # @!visibility private
      def inspect
        "<Trigger type=#{@type}>"
      end
    end

    # An action that will execute whenever an automod rule is triggered.
    class Action
      # Mapping of action types.
      TYPES = {
        block_message: 1,
        send_alert_message: 2,
        timeout_member: 3,
        block_member_interaction: 4
      }.freeze

      # @return [Integer] the type of the action.
      attr_reader :type

      # @return [String, nil] the message shown to members when their message is blocked.
      attr_reader :custom_message

      # @return [Integer, nil] The ID of the channel where user content should be logged.
      attr_reader :alert_channel_id

      # @return [Integer] the duration (in seconds) for how long the member's timeout will last.
      attr_reader :timeout_duration

      # @!visibility private
      def initialize(data, bot)
        @bot = bot
        @type = data[:type]
        @custom_message = data[:metadata][:custom_message]
        @alert_channel_id = data[:metadata][:channel_id]&.to_i
        @timeout_duration = data[:metadata][:duration_seconds] || 0
      end

      # Get the channel where user content will be logged.
      # @return [Channel, nil] The channel where user content will be logged.
      def alert_channel
        @bot.channel(@alert_channel_id) if @alert_channel_id
      end

      # @!method block_message?
      #   @return [true, false] whether or not this action will block a message.
      # @!method send_alert_message?
      #   @return [true, false] whether or not this action will send an alert message.
      # @!method timeout_member?
      #   @return [true, false] whether or not the action will timeout a guild member.
      # @!method block_member_interaction?
      #   @return [true, false] whether or not the action will block a member from interacting in a guild.
      TYPES.each do |name, value|
        define_method("#{name}?") { @type == value }
      end

      # Check if two auto moderation action objects are equivalent.
      # @param other [Object] The object to compare against for equality.
      # @return [true, false] Whether or not the two objects are equivalent.
      def ==(other)
        return false unless other.is_a?(Action)

        @type == other.type && @custom_message == other.custom_message &&
          @alert_channel_id == other.alert_channel_id && @timeout_duration == other.timeout_duration
      end

      alias_method :eql?, :==

      # @!visibility private
      def to_h
        {
          type: @type,
          metadata: {
            custom_message: @custom_message,
            channel_id: @alert_channel_id,
            duration_seconds: @timeout_duration
          }.compact
        }
      end

      # @!visibility private
      def inspect
        "<Action type=#{@type}>"
      end

      # Builder for actions.
      class Builder
        # @!visibility private
        def initialize
          @actions = []
        end

        # Add an action to the builder.
        # @param type [Integer, String, Symbol] The type of the action to create. See {Action::TYPES}.
        # @param alert_channel [Integer, String, Channel, nil] The channel to which user content should be logged.
        # @param timeout_duration [Integer, nil] The duration of the timeout in seconds.
        # @param custom_message [String, nil] The additional explanation that will be shown to members when their message is blocked.
        # @note Certain types require specific arguments to be passed. To learn which types require which arguments to be passed, please refer to:
        #   https://discord.com/developers/docs/resources/auto-moderation#auto-moderation-action-object
        def action(type, alert_channel: nil, timeout_duration: nil, custom_message: nil)
          metadata = { channel_id: alert_channel&.resolve_id, duration_seconds: timeout_duration, custom_message: custom_message }.compact

          @actions << { type: type.is_a?(Numeric) ? type : Action::TYPES[type.to_sym], metadata: metadata.empty? ? nil : metadata }.compact
        end

        alias_method :add_action, :action

        # @!visibility private
        def to_a
          @actions.uniq { |action| action[:type] }
        end
      end
    end
  end
end
