...
module Discordrb
  ...
  class AutoModRule
    ...
    # @return [Server] the server this automod rule belongs from.
    def server
    ...
    def inspect
      "<AutoModRule name=#{@name} creator_id=#{@creator_id} server_id=#{@server_id} trigger=#{@trigger.to_h} actions=#{@actions.map(&:to_h)}>"
    end
    ...
    # Information used to determine when a rule should be triggered.
    class Trigger
