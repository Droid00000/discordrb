...
module Discordrb
  ...
  class Server
    ...
    def create_soundboard_sound(name:, file:, volume: nil, emoji: nil, reason: nil)
      ...
      Sound.new(JSON.parse(response), @bot).tap { |sound| cache_soundboard_sound(sound) }
    end

    # Get a list of soundboard sounds in the server.
    # @param bypass_cache [true, false] Whether to bypass the cache and re-fetch all soundboard sounds via HTTPS.
    ...
    # @param sound_id [Integer, String, Sound] The ID of the soundboard sound to fetch.
    ...
    def soundboard_sound(sound_id, request: true)
      id = sound_id.resolve_id
      return @soundboard_sounds[id] if @soundboard_sounds[id] || !request
