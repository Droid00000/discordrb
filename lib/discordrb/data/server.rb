...
module Discordrb
  ...
  class Server
    ...
    # @param type [Integer, Symbol] The entity type of the scheduled event to create.
    ...
    def create_scheduled_event(name:, starts_at:, type:, ends_at: nil, channel: nil, location: nil, description: nil, cover: nil, recurrence_rule: nil, reason: nil)
      type = ScheduledEvent::TYPES[type] || type
      ...
      data = JSON.parse(API::Server.create_scheduled_event(@bot.token, @id, name, 2, starts_at.iso8601, type, channel&.resolve_id, entity_metadata, ends_at&.iso8601, description, cover, recurrence_rule, reason))
