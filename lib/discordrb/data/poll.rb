...
module Discordrb
  ...
  class Poll
    ...
    # @return [Time, nil] the time at when this poll will expire.
    ...
    # @return [true, false] whether this poll has expired and the results have
    ...
    def initialize(data, message, bot)
      ...
      @expires_at = data['expiry'] ? Time.parse(data['expiry']) : nil
    ...
    # Whether this poll has expired.
    # @return [true, false] whether or not this poll has expired.
    def expired?
    ...
    class Answer
      ...
      def voters(limit: 100)
        get_voters = proc do |fetch_limit, after = nil|
          ...
          JSON.parse(response)['users'].map { |user| @bot.ensure_user(user) }
        end
      ...
      # @!visibility private
      def inspect
        "<Answer id=#{@id} text=#{@text} votes=#{@votes} emoji=#{@emoji&.inspect}>"
      end
    end
