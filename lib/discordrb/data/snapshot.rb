...
module Discordrb
  # A message snapshot is a minimal message object that only contains a small subset of fields present on full
  #   message objects. All fields are copied over to the snapshot at the time of forwarding and do **not** live
  #   update when the original message is modified or deleted.
  class Snapshot
    # @return [Integer] the type of the message snapshot.
    attr_reader :type

    # @return [String] the content of the message snapshot.
    attr_reader :content
    ...
    def initialize(data, bot)
      ...
      @attachments = data["attachments"]&.map { |attach| Attachment.new(attach, self, bot) } || []
      ...
      @mentions = data["mentions"]&.map { |mention| @mentions << bot.ensure_user(mention) } || []
    ...
    # Get the custom emojis that were used in the snapshot.
    # @return [Array<Emoji>] the emojis used in the snapshot.
    def emojis
      return [] if @content.nil? || @content.empty?

      @emojis ||= @bot.parse_mentions(@content).select { |e| e.is_a?(Discordrb::Emoji) }
    end

    # Get the roles that were mentioned in the snapshot.
    # @return [Array<Role>] the roles that were mentioned in the snapshot.
    # @note this can only resolve roles in servers that the bot can access.
    def role_mentions
    ...
    # Get the buttons that were used in the snapshot.
    # @return [Array<Components::Button>] the button components used in the snapshot.
    def buttons
