...
module Discordrb
  ...
  class ClientTheme
    ...
    # @return [Array<Color>] the 1-5 colors of the theme.
    attr_reader :colours
    ...
    def initialize(data, bot)
      ...
      @colours = data[:colors]&.map { |value| Color.new(value) }
    end
    ...
    def ==(other)
      return false unless other.is_a?(ClientTheme)
    ...
    def to_h
      {
        ...
        colors: @colours.map { |color| format('%06x', color.to_i) }
      }
    end

    # @!visibility private
    def inspect
      "<ClientTheme base=#{@base} angle=#{@angle} intensity=#{@intensity}>"
    end
    ...
    class Builder
      ...
      # @param value [Integer, String, Color] The colour to add to the theme's colours.
      # @return [void]
      def colour(value)
