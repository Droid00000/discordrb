...
module Discordrb
  ...
  class ScheduledEvent
    ...
    # @return [Integer] the entity type of the scheduled event.
    attr_reader :type

    # @return [String] the 1-100 character name of the scheduled event.
    attr_reader :name
    ...
    # Set the type of the scheduled event to something new.
    # @param type [Symbol, Integer] the new type of the scheduled event.
    def type=(type)
      update_data(entity_type: TYPES[type] || type)
    end
    ...
    # Set the type of the scheduled event. You must use this method
    #   instead of {#type=} when setting the type to `:external`.
    # @param type [Symbol, Integer] the new type of the scheduled event.
    # @param location [String, nil] the new location of the scheduled event.
    # @param ends_at [Time, Date] the new ending time of the scheduled event.
    # @param channel [Channel, Integer, String, nil] the new channel of the scheduled event.
    # @param reason [String, nil] the reason that will show up for modifying the scheduled event.
    def update_type(type:, channel: :undef, location: :undef, ends_at: :undef, reason: nil)
      new_data = {
        reason: reason,
        entity_type: TYPES[type] || type,
    ...
    # @return [Array<User, Member>] the users or members that are subscribed to the scheduled event.
    def subscribers(limit: 100, member: false)
      get_users = proc do |limit_, after = nil|
        ...
        response.map { |data| data['member'] ? Member.new(data['member'], @server, @bot) : User.new(data['user'], @bot) }
      end
    ...
    # @!visibility private
    def inspect
      "<ScheduledEvent id=#{@id} name=\"#{@name}\" status=#{@status} type=#{@type} starts_at=#{@starts_at} ends_at=#{@ends_at}"
    end
    ...
    class RecurrenceRule
      ...
      # @!attribute week
      #   @return [Integer] The week (1-5) to reoccur on.
      # @!attribute day
      #   @return [Integer] The day of the week to reoccur on.
