...
describe Discordrb::SKU do
  ...
  describe '#guild_subscription?' do
    it 'returns if the sku is a guild subscription' do
      expect(sku.guild_subscription?).to eq sku_data[:flags].anybits?(Discordrb::SKU::FLAGS[:guild_subscription])
    end
  end
