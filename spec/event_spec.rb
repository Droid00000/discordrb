    ...
    shared_examples 'end_with attributes' do |expr, matching, non_matching|
      describe 'end_with attribute' do
        it "matches #{matching}" do
          ...
          event = double('event', channel: double('channel', private?: false), author: double('author'), timestamp: double('timestamp'), content: matching, message: double('message', type: 0), guild: true)
        ...
        it "doesn't match #{non_matching}" do
          handler = Discordrb::Events::MessageEventHandler.new({ end_with: expr }, double('proc'))
          event = double('event', channel: double('channel', private?: false), author: double('author'), timestamp: double('timestamp'), content: non_matching, message: double('message', type: 0), guild: true)
          allow(event).to receive(:is_a?).with(Discordrb::Events::MessageEvent).and_return(true)
